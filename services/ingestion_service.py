...
async def ingest_directory(dir_path: str, kg_id: str | None = None) -> tuple[list[Document], list[str]]:
    """
    批次匯入目錄下所有支援格式的文件。
    若指定 kg_id，匯入後自動建立 KG → Document 的 CONTAINS 關係。
    回傳 (成功列表, 失敗訊息列表)。
    """
    ...
    kg_uuid: _UUID | None = None
    kg_repo = None
    if kg_id:
        try:
            kg_uuid = _UUID(kg_id)
        except ValueError:
            logger.warning(f"kg_id 格式錯誤，將略過 KG 關聯：{kg_id}")
        else:
    ...
    for f in files:
        try:
            ...
            success.append(doc)
            if kg_repo and kg_uuid:
                await kg_repo.add_document(kg_uuid, doc.id)
                logger.info(f"✅ 匯入成功並關聯至 KG：{f.name}")
            else:
