...
* **學術文獻**：*He, X., et al. (2023). "Mind's Eye of LLM: Reasoning on Graphs with Chain-of-Thought."* arXiv:2310.13344

### ④ 主動自適應檢索 (Active & Adaptive Retrieval)
* **當前局限**：現有機制為單次檢索後生成答案，即便有自我精煉（Self-Refinement）也只是被動回填 Chunks，無法在生成過程中自發性地決定何時需要新知識。
* **優化建議**：引入 **Active RAG (主動式檢索增強)**。在 LLM 串流生成的過程中，如果發現缺失某個中間邏輯鏈條的知識，能自發發起圖譜檢索，實現自適應生成。
* **學術文獻**：*Asai, A., et al. (ICLR 2024). "Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection."*
